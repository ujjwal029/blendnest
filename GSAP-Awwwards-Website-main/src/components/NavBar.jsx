...
const NavBar = ({ cartCount, session }) => {
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 md:px-9 md:py-6 px-4 py-3 flex justify-between items-center bg-black/20 backdrop-blur-md">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden md:flex items-center gap-4">
          {/* Show user info or login button based on session */}
          {session ? (
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm bg-red-500/20 hover:bg-red-500/30 text-white rounded-lg transition-all duration-200"
              >
                Logout
              </button>
            </div>
        ...
        <button
          ...
          className="flex flex-col gap-1.5 p-2 -mr-2 text-dark-brown hover:opacity-80 transition-opacity focus:outline-none focus:ring-2 focus:ring-dark-brown/30 rounded-md md:hidden"
        ...
        {isOpen && (
          <>
            <div
              ...
              aria-hidden="true"
            />
            <div className="absolute right-4 top-full mt-2 py-4 min-w-[200px] bg-milk rounded-xl shadow-xl border border-[#c8e6c9] z-50 md:hidden animate-in slide-in-from-top-2 duration-200">
              <div className="flex flex-col gap-3 px-6 py-3 border-b border-[#c8e6c9]">
                {/* Mobile menu: User info or Login */}
                {session ? (
                  <div className="flex flex-col gap-2">
                    <span className="text-sm font-medium text-dark-brown truncate">
                      {session.user.email || session.user.user_metadata?.name}
                    </span>
                    <button
                      onClick={handleLogout}
                      className="px-4 py-2 text-sm bg-red-500/20 hover:bg-red-500/30 text-white rounded-lg transition-all w-full text-center"
                    >
                      Logout
                    </button>
                  </div>
